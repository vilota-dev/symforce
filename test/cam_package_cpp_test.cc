...
//     tests/cam_package_cpp_test.cc.jinja
...
TEMPLATE_TEST_CASE("Test group ops", "[cam_package]", sym::ATANCameraCal<double>,
  ...
  CHECK(identity.IsApprox(
      sym::GroupOps<T>::ComposeWithJacobians(identity, identity, &jacobian1, &jacobian2), 1e-9));
...
TEMPLATE_TEST_CASE("Test Lie group ops", "[cam_package]", sym::ATANCameraCal<double>,
  ...
  CHECK(
      identity.IsApprox(sym::LieGroupOps<T>::FromTangent(TangentVec::Zero(), sym::kDefaultEpsilond),
                        sym::kDefaultEpsilond));

  CHECK(sym::LieGroupOps<T>::ToTangent(identity, sym::kDefaultEpsilond).norm() <
        sym::kDefaultEpsilond);
  CHECK(identity.IsApprox(
      sym::LieGroupOps<T>::Retract(identity, TangentVec::Zero(), sym::kDefaultEpsilond),
      sym::kDefaultEpsilond));
  CHECK(sym::LieGroupOps<T>::LocalCoordinates(identity, identity, sym::kDefaultEpsilond).norm() <
        sym::kDefaultEpsilond);
}
...
TEMPLATE_TEST_CASE("Test EquirectangularCameraCal named constructor", "[cam_package]", double,
                   float) {
  using Scalar = TestType;
...
TEMPLATE_TEST_CASE("Test PolynomialCameraCal named constructor", "[cam_package]", double, float) {
  ...
  const Eigen::Matrix<Scalar, 3, 1> distortion_coeffs = Eigen::Matrix<Scalar, 3, 1>(0.1, 0.2, 0.3);
